...
# v2.0.0 Refactor
# Purpose:
#   Clean baseline for maze DQN.
#   - No stage curriculum.
#   - Fixed mixed difficulty distribution: easy 0.4 / medium 0.3 / hard 0.3.
#   - Uniform replay by default.
#   - No wall/success/progress/recent stratified replay.
#   - No manual mazes in training.
#   - Optional demo buffer and optional large-margin loss are kept as ablation switches.
#   - Target mode is configurable: double / target / none.
#   - --test runs only the canonical fixed hand-written maze.
# ============================================================

VERSION = "v2.0.0_refactor"
SIZE = 8
...
@dataclass
class TrainConfig:
    output_dir: str = "./v2.0.0_refactor"
    output_name: str = "v2.0.0_refactor_uniform_mixed"
    seed: int = 42
    ...
    # Target mode:
    #   double: Double DQN target, target net evaluates selected policy action.
    #   target: vanilla DQN target, target net selects/evaluates max action.
    #   none: no target net, policy net selects/evaluates max action.
...
def apply_reward_preset(cfg: TrainConfig) -> None:
    ...
    cfg.goal_reward, cfg.wall_penalty, cfg.step_penalty = REWARD_PRESETS[cfg.reward_preset]
...
def train(cfg: TrainConfig) -> Tuple[CNN_DQN, Dict[str, Any]]:
    ...
    print("\n=== Train v2.0.0 Refactor ===")
    print(f"Device: {device}")
    print("Kept: CNN-DQN, Double/Target/No-target selectable TD, random easy/medium/hard mazes.")
    print("Deleted: stage curriculum, manual training, stratified replay, wall/success/progress/recent buffers.")
    print("Optional ablations: demo buffer and margin loss are disabled unless requested.")
    print(f"difficulty_mix: easy={cfg.easy_ratio}, medium={cfg.medium_ratio}, hard={cfg.hard_ratio}")
    print(f"target_mode={cfg.target_mode}, use_demo={cfg.use_demo}, use_margin={cfg.use_margin}, margin_source={cfg.margin_source}")
...
def build_config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    if cfg.output_name == "v2.0.0_refactor_uniform_mixed":
        parts = ["v2.0.0_refactor", cfg.target_mode]
        if cfg.use_demo:
            parts.append("demo")
        if cfg.use_margin:
            parts.append(f"margin_{cfg.margin_source}")
        cfg.output_name = "_".join(parts)
    return cfg
...
def main() -> None:
    ...
    p.add_argument("--mode", choices=["single"], default="single")
    ...
    p.add_argument("--replay-size", type=int, dest="replay_size")
    p.add_argument("--warmup-steps", type=int, dest="warmup_steps")
    p.add_argument("--updates-per-episode", type=int, dest="updates_per_episode")
    p.add_argument("--target-mode", choices=["double", "target", "none"], dest="target_mode")
    p.add_argument("--target-update-interval", type=int, dest="target_update_interval")
    p.add_argument("--epsilon-start", type=float, dest="epsilon_start")
    p.add_argument("--epsilon-end", type=float, dest="epsilon_end")
    p.add_argument("--epsilon-decay-episodes", type=int, dest="epsilon_decay_episodes")
    p.add_argument("--easy-ratio", type=float, dest="easy_ratio")
    p.add_argument("--medium-ratio", type=float, dest="medium_ratio")
    p.add_argument("--hard-ratio", type=float, dest="hard_ratio")
    p.add_argument("--reward-preset", type=str, dest="reward_preset", choices=list(REWARD_PRESETS.keys()))
    p.add_argument("--use-demo", action="store_true", dest="use_demo")
    p.add_argument("--demo-ratio", type=float, dest="demo_ratio")
    p.add_argument("--demo-per-episode", type=int, dest="demo_per_episode")
    p.add_argument("--use-margin", action="store_true", dest="use_margin")
    p.add_argument("--margin", type=float)
    p.add_argument("--margin-lambda", type=float, dest="margin_lambda")
    p.add_argument("--margin-source", choices=["demo", "bfs_online"], dest="margin_source")
    p.add_argument("--eval-n", type=int, dest="eval_n")
    p.add_argument("--de maze_dqn_v2_0_1.pyvice", type=str)
    args = p.parse_args()
