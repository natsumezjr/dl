...
# ============================================================
# v2.0.2 CEM Reward Ratio Learning
# Purpose:
...
VERSION = "v2.0.2_cem_reward_learning"
SIZE = 8
...
@dataclass
class TrainConfig:
    output_dir: str = "./v2.0.2_cem_reward_learning"
    output_name: str = "v2.0.2_cem_reward_learning"
    seed: int = 42
...
def train(cfg: TrainConfig) -> Tuple[CNN_DQN, Dict[str, Any]]:
    ...
    print("\n=== Train v2.0.2 CEM Reward Ratio Learning ===")
    ...
    print("No demo, no BFS oracle, no margin. Candidate reward is generated from normalized ratios (g,w).")
...
class CEMConfig:
    ...
    so the practical environment reward becomes:
        goal = +G, wall = -W, step = -c.
    """
    generations: int = 4
    population: int = 6
    elite: int = 2
    candidate_episodes: int = 3000
    ...
    # Example: g=32, w=16, step_scale=2 -> goal=64, wall=-32, step=-2.
...
def force_clean_v202_config(cfg: TrainConfig) -> TrainConfig:
    """v2.0.2 CEM line is clean: Double DQN, uniform replay, no demo/BFS/margin."""
...
def build_ratio_config(
    ...
    if final:
        cfg.output_name = (
            f"v2.0.2_cem_final_"
            f"g{clean_number(g)}_w{clean_number(w)}_scale{clean_number(cem.step_scale)}"
        )
    else:
        cfg.output_name = (
            f"v2.0.2_cem_gen{generation:02d}_cand{candidate_id:02d}_"
            f"g{clean_number(g)}_w{clean_number(w)}"
        )
    return cfg
...
def sample_cem_candidates(
    ...
    for i in range(cem.population):
        ...
        candidates.append({"candidate_id": i, "g": g, "w": w, "log_g": math.log(g), "log_w": math.log(w)})
    return candidates
...
def trajectory_quality_score(
    ...
    return float(-cem.timeout_penalty)
...
def update_cem_distribution(
    ...
    elite_logs = np.asarray([[float(e["log_g"]), float(e["log_w"])] for e in elites], dtype=np.float64)
...
def write_cem_summary(out_dir: Path, summary: Dict[str, Any]) -> None:
    ...
    json_path = out_dir / "v2.0.2_cem_summary.json"
    ...
    for gen in summary.get("generations", []):
        for cand in gen.get("candidates", []):
            ...
            rows.append(row)
    csv_path = out_dir / "v2.0.2_cem_summary.csv"
    if rows:
...
def run_cem_reward_learning(base_cfg: TrainConfig, cem: CEMConfig) -> Dict[str, Any]:
    ...
    mu = np.asarray([math.log(cem.init_g), math.log(cem.init_w)], dtype=np.float64)
    sigma = np.asarray([cem.init_sigma, cem.init_sigma], dtype=np.float64)
    ...
    print(f"initial normalized ratios: g={cem.init_g}, w={cem.init_w}, sigma={cem.init_sigma}")
    print(f"actual scale: goal=step_scale*g, wall=-step_scale*w, step=-step_scale; step_scale={cem.step_scale}")

    global_best: Optional[Dict[str, Any]] = None
    for generation in range(cem.generations):
        print(f"\n=== CEM Generation {generation+1}/{cem.generations} ===")
        print(f"distribution center: g={math.exp(mu[0]):.4f}, w={math.exp(mu[1]):.4f}, sigma_log=({sigma[0]:.4f},{sigma[1]:.4f})")
        ...
        for cand in candidates:
            ...
            g = float(cand["g"])
            w = float(cand["w"])
            cfg = build_ratio_config(base_cfg, cem, g, w, generation, cid, cem.candidate_episodes, final=False)
            cfg.seed = int(base_cfg.seed + generation * 1000 + cid)
            cfg.save_gif = False
            print(f"\n--- candidate gen={generation} id={cid}: g={g:.4f}, w={w:.4f}, actual=(G={cfg.goal_reward:.4f}, W={-cfg.wall_penalty:.4f}, c={-cfg.step_penalty:.4f}) ---")
            model, obj = train(cfg)
        ...
        gen_obj = {
            ...
            "old_center": {"g": math.exp(old_mu[0]), "w": math.exp(old_mu[1])},
            "new_mu_log": mu.tolist(),
            "new_sigma_log": sigma.tolist(),
            "new_center": {"g": math.exp(mu[0]), "w": math.exp(mu[1])},
        ...
        print(f"[CEM update] center -> g={math.exp(mu[0]):.4f}, w={math.exp(mu[1]):.4f}; sigma=({sigma[0]:.4f},{sigma[1]:.4f})")

    learned_g = float(math.exp(mu[0]))
    learned_w = float(math.exp(mu[1]))
    print("\n=== Final Training with Learned Reward Ratios ===")
    print(f"learned normalized ratios: g={learned_g:.4f}, w={learned_w:.4f}")
    final_cfg = build_ratio_config(base_cfg, cem, learned_g, learned_w, cem.generations, 0, cem.final_episodes, final=True)
...
def build_base_config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    cfg.output_dir = args.output_dir or "./v2.0.2_cem_reward_learning"
    cfg.output_name = args.output_name or "v2.0.2_cem_reward_learning"
...
def main() -> None:
    ...
    p.add_argument("--mode", choices=["cem", "final"], default="cem", help="cem learns reward ratios; final trains once from given final-g/final-w.")
    p.add_argument("--test", action="store_true", help="Run only the fixed canonical test maze.")
    p.add_argument("--model", type=str, default="")

    # Base DQN controls.
    p.add_argument("--output-dir", type=str, default="./v2.0.2_cem_reward_learning")
    ...
    if args.mode == "final":
        if args.final_g is None or args.final_w is None:
            raise ValueError("--mode final requires --final-g and --final-w")
        cfg = build_ratio_config(base_cfg, cem_cfg, float(args.final_g), float(args.final_w), 0, 0, cem_cfg.final_episodes, final=True)
        model, _ = train(cfg)
