"""
maze_dqn_v2_0_7_family_margin_preference_rm.py

v2.0.7: Family + Bucketed Margin Preference Reward Model -> QCNN
...
VERSION = "v2.0.7_family_margin_preference_rm"
SIZE = 8
...
def validate_trajectory(tr: Trajectory, label: str, args: argparse.Namespace, strict: bool = True) -> bool:
    ...
    if label == "bfs_success":
        ...
        ok = tr.success and tr.gap > 0 and tr.wall_hits <= args.success_explore_max_wall and tr.visit3plus_steps <= args.success_explore_max_visit3plus
    elif label == "recovery_success":
        ok = tr.success and tr.visit2_steps >= 1 and tr.visit3plus_steps <= args.recovery_max_visit3plus and tr.wall_hits <= args.recovery_max_wall and tr.gap > 0
    elif label == "clean_timeout":
        ok = (not tr.success) and tr.wall_hits == 0 and tr.visit3plus_steps <= args.clean_timeout_max_visit3plus
    elif label == "stuck_timeout":
        ok = (not tr.success) and tr.wall_hits == 0 and tr.visit3plus_steps >= args.stuck_min_visit3plus and tr.max_visit >= args.stuck_min_max_visit
    elif label == "wall_timeout":
        ok = (not tr.success) and tr.wall_hits >= 1
    elif label == "repeated_wall_timeout":
        ok = (not tr.success) and tr.wall_hits >= args.repeated_wall_min_hits and tr.max_visit >= args.repeated_wall_min_max_visit
    else:
        ok = True
...
def run_key_unit_tests(args: argparse.Namespace) -> None:
    ...
    # visit2_recovery > visit3plus_stuck because success beats timeout and low visit beats high visit.
    visit2_recovery = synthetic_key(True, False, 0, 18, 10, 0, 10, 0, 0.03, args)
    visit3_stuck = synthetic_key(False, False, 8, 64, 10, 1, 10, 0, 0.5, args)
    assert visit2_recovery > visit3_stuck, "key test failed: visit2_recovery > visit3plus_stuck"
...
def generate_clean_timeout_branch(
    grid: np.ndarray,
    start: Tuple[int, int],
    goal: Tuple[int, int],
    prefix: List[int],
    ...
    for _ in range(args.generator_retry):
        ...
        for _step in range(args.max_steps - len(prefix)):
            ...
            if len(actions) >= args.max_steps:
                break
        tr = run_actions_as_trajectory(grid, start, goal, actions, "clean_timeout", args, maze_id, family_id, prefix, branch)
        if validate_trajectory(tr, "clean_timeout", args, strict=False):
            return tr
    return None


def generate_stuck_timeout_branch(
    grid: np.ndarray,
    start: Tuple[int, int],
    goal: Tuple[int, int],
    prefix: List[int],
    ...
    for a in acts:
        ...
        tr = run_actions_as_trajectory(grid, start, goal, actions, "stuck_timeout", args, maze_id, family_id, prefix, branch)
        if validate_trajectory(tr, "stuck_timeout", args, strict=False):
            return tr
    return None
...
def build_family(
    ...
    for gen_fn in [
        ...
        generate_clean_timeout_branch,
        generate_stuck_timeout_branch,
    ]:
        tr = gen_fn(grid, start, goal, prefix, args, maze_id, family_id)
        if tr is not None:
            trajs.append(tr)
    trw = generate_wall_timeout_branch(grid, start, goal, prefix, args, maze_id, family_id, repeated=False)
    if trw is not None:
        trajs.append(trw)
    trrw = generate_wall_timeout_branch(grid, start, goal, prefix, args, maze_id, family_id, repeated=True)
    if trrw is not None:
        trajs.append(trrw)
...
BUCKETS = ["outcome", "wallPresence", "wallCount", "visit", "path", "cpl", "random"]
...
def component_for_bucket(tr: Trajectory, bucket: str) -> float:
    if bucket == "outcome":
        return tr.b_outcome
    if bucket == "wallPresence":
        return tr.b_wall_presence
    if bucket == "wallCount":
        return tr.b_wall_count
    if bucket == "visit":
        return tr.b_visit
    if bucket == "path":
        return tr.b_path
...
def bucket_margin_params(args: argparse.Namespace, bucket: str) -> Tuple[float, float, float]:
    ...
    if bucket == "wallCount":
        return args.margin_wall_count_min, args.margin_wall_count_max, args.margin_wall_count_s
    if bucket == "visit":
        return args.margin_visit_min, args.margin_visit_max, args.margin_visit_s
    if bucket == "path":
        return args.margin_path_min, args.margin_path_max, args.margin_path_s
    if bucket == "cpl":
        return args.margin_cpl_min, args.margin_cpl_max, args.margin_cpl_s
    return args.margin_path_min, args.margin_path_max, args.margin_path_s
...
def generate_pair_candidates_for_maze(families: List[Family], args: argparse.Namespace) -> Dict[str, List[PreferencePair]]:
    ...
    all_trajs = [t for fam in families for t in fam.trajectories]
    # Family mandatory candidates.
    for fam in families:
        ts = fam.trajectories
        bfs = next(iter(find_by_source(ts, "bfs_success")), None)
        sx = next(iter(find_by_source(ts, "success_explore")), None)
        rec = next(iter(find_by_source(ts, "recovery_success")), None)
        clean = next(iter(find_by_source(ts, "clean_timeout")), None)
        stuck = next(iter(find_by_source(ts, "stuck_timeout")), None)
        wall = next(iter(find_by_source(ts, "wall_timeout")), None)
        rw = next(iter(find_by_source(ts, "repeated_wall_timeout")), None)
        # outcome
        for s in [bfs, sx, rec]:
            for f in [clean, stuck, wall, rw]:
                add_candidate_pair(cands, s, f, "outcome", "mandatory", args)
        # wall presence
        add_candidate_pair(cands, clean, wall, "wallPresence", "mandatory", args)
        add_candidate_pair(cands, clean, rw, "wallPresence", "mandatory", args)
        # wall count
        add_candidate_pair(cands, wall, rw, "wallCount", "mandatory", args)
        # visit
        add_candidate_pair(cands, rec, stuck, "visit", "mandatory", args)
        add_candidate_pair(cands, clean, stuck, "visit", "mandatory", args)
        # path
        add_candidate_pair(cands, bfs, sx, "path", "mandatory", args)
        add_candidate_pair(cands, bfs, rec, "path", "mandatory", args)
        # cpl
        add_candidate_pair(cands, rec, wall, "cpl", "mandatory", args)
        add_candidate_pair(cands, rec, rw, "cpl", "mandatory", args)
        add_candidate_pair(cands, bfs, wall, "cpl", "mandatory", args)
        add_candidate_pair(cands, sx, stuck, "cpl", "mandatory", args)
    # Additional candidates from all trajectories, bucketed by first differing key component.
    for i in range(len(all_trajs)):
        for j in range(i + 1, len(all_trajs)):
            a, b = all_trajs[i], all_trajs[j]
            bw = better_worse(a, b)
            if bw is None:
                continue
            pos, neg = bw
            if pos.success != neg.success:
                bucket = "outcome"
            elif (pos.wall_hits > 0) != (neg.wall_hits > 0):
                bucket = "wallPresence"
            elif pos.wall_hits != neg.wall_hits and pos.wall_hits > 0 and neg.wall_hits > 0:
                bucket = "wallCount"
            elif visit_tier(pos, args) != visit_tier(neg, args) or abs(pos.b_visit - neg.b_visit) > 1e-5:
                bucket = "visit"
            else:
                bucket = "path"
            cands[bucket].append(make_pair(pos, neg, bucket, "random_candidate", args))
    return cands


def sample_bucket_pairs(cands: Dict[str, List[PreferencePair]], args: argparse.Namespace) -> List[PreferencePair]:
    targets = {
        "outcome": args.outcome_pairs_per_maze,
        "wallPresence": args.wall_presence_pairs_per_maze,
        "wallCount": args.wall_count_pairs_per_maze,
        "visit": args.visit_pairs_per_maze,
        "path": args.path_pairs_per_maze,
        "cpl": args.cpl_pairs_per_maze,
    }
    selected: List[PreferencePair] = []
    seen = set()
    for bucket, n in targets.items():
        pool = cands.get(bucket, [])
        random.shuffle(pool)
        take = pool[: min(n, len(pool))]
        for p in take:
            key = (id(p.pos), id(p.neg), p.bucket)
            if key not in seen:
                seen.add(key)
                selected.append(p)
    # Fill to pairs_per_maze with bucket-first random sampling.
    tries = 0
    while len(selected) < args.pairs_per_maze and tries < args.pairs_per_maze * 20:
        tries += 1
...
def print_dataset_reports(trajectories: List[Trajectory], pairs: List[PreferencePair], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    order = ["bfs_success", "success_explore", "recovery_success", "clean_timeout", "stuck_timeout", "wall_timeout", "repeated_wall_timeout"]
    ...
    if by_type.get("clean_timeout") and by_type.get("stuck_timeout") and mean_b("stuck_timeout") <= mean_b("clean_timeout"):
        print("[WARN] stuck_timeout B_total is not higher than clean_timeout.")
    if by_type.get("wall_timeout") and by_type.get("clean_timeout") and mean_b("wall_timeout") <= mean_b("clean_timeout"):
        print("[WARN] wall_timeout B_total is not higher than clean_timeout.")
...
def rollout_policy(
    ...
    metrics = {
        ...
        "clean_timeout": float((not success) and env.wall_hits == 0),
...
def print_eval(eval_dict: Dict[str, Dict[str, float]]) -> None:
    ...
    for name, m in eval_dict.items():
        print(f"{name}")
        print(f"  outcome : success={fmt_pct(m['success'])} clean_timeout={fmt_pct(m['clean_timeout'])} wall_timeout={fmt_pct(m['wall_timeout'])}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output-dir", default="./v2.0.7")
    ...
    # dataset
    p.add_argument("--rm-mazes", type=int, default=600)
    ...
    p.add_argument("--success-explore-max-visit3plus", type=int, default=2)
    p.add_argument("--success-explore-max-extra", type=int, default=6)
    p.add_argument("--recovery-max-visit3plus", type=int, default=1)
    p.add_argument("--recovery-max-wall", type=int, default=1)
    p.add_argument("--clean-timeout-max-visit3plus", type=int, default=2)
    p.add_argument("--stuck-min-visit3plus", type=int, default=8)
    p.add_argument("--stuck-min-max-visit", type=int, default=4)
    p.add_argument("--repeated-wall-min-hits", type=int, default=8)
    p.add_argument("--repeated-wall-min-max-visit", type=int, default=4)
    # pairs
    p.add_argument("--pairs-per-maze", type=int, default=40)
    p.add_argument("--mandatory-pairs-per-maze", type=int, default=24)
    p.add_argument("--random-pairs-per-maze", type=int, default=16)
    p.add_argument("--outcome-pairs-per-maze", type=int, default=10)
    p.add_argument("--wall-presence-pairs-per-maze", type=int, default=6)
    p.add_argument("--wall-count-pairs-per-maze", type=int, default=6)
    p.add_argument("--visit-pairs-per-maze", type=int, default=6)
    p.add_argument("--path-pairs-per-maze", type=int, default=4)
    p.add_argument("--cpl-pairs-per-maze", type=int, default=8)
    # margins
    p.add_argument("--margin-outcome-min", type=float, default=0.72)
    p.add_argument("--margin-outcome-max", type=float, default=0.95)
    p.add_argument("--margin-outcome-s", type=float, default=1.00)
    p.add_argument("--margin-cpl-min", type=float, default=0.42)
    p.add_argument("--margin-cpl-max", type=float, default=0.70)
    p.add_argument("--margin-cpl-s", type=float, default=0.20)
    p.add_argument("--margin-wall-presence-min", type=float, default=0.35)
    p.add_argument("--margin-wall-presence-max", type=float, default=0.68)
    p.add_argument("--margin-wall-presence-s", type=float, default=1.00)
    p.add_argument("--margin-visit-min", type=float, default=0.18)
    p.add_argument("--margin-visit-max", type=float, default=0.45)
    p.add_argument("--margin-visit-s", type=float, default=0.10)
    p.add_argument("--margin-wall-count-min", type=float, default=0.15)
    p.add_argument("--margin-wall-count-max", type=float, default=0.40)
    p.add_argument("--margin-wall-count-s", type=float, default=0.10)
    p.add_argument("--margin-path-min", type=float, default=0.03)
    p.add_argument("--margin-path-max", type=float, default=0.22)
    p.add_argument("--margin-path-s", type=float, default=0.25)
    # RM training
    p.add_argument("--rm-epochs", type=int, default=8)
    p.add_argument("--rm-batch-size", type=int, default=32)
    p.add_argument("--rm-lr", type=float, default=1e-3)
    p.add_argument("--reward-l2", type=float, default=0.01)
    p.add_argument("--score-normalizer", choices=["bfs_len", "episode_len", "raw"], default="bfs_len")
...
def main() -> None:
    ...
    print("Changed: trajectory families, validators, explicit visit fields, bucketed tanh margins, structured pair budget.")
    if args.test:
