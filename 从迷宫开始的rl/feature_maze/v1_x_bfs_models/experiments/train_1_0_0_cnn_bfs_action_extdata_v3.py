...
VERSION = "1.0.0_data_swap_3_0_4_extdata_v3"
...
DEFAULTS: Dict[str, Any] = dict(
    ...
    max_train_mazes=None,
    train_distribution_policy="source_order",
    train_distribution_seed=42,
    handdraw_split="all",
    ...
    handdraw_dataset_path=str(LEARNING_ROOT / "从迷宫开始的rl/handdraw_mazes_ascii.jsonl"),
    ...
    sample_check_run_name="default_sample_check_extdata_v3",
    handdraw_check_run_name="default_handdraw_check_extdata_v3",
...
def train_model(model, train_loader, val_loader, args, device, out_dir):
    ...
    best_loss=1e9; best_path=out_dir/"1_0_0_extdata_v3_cnn_bfs_action_best.pt"; last_path=out_dir/"1_0_0_extdata_v3_cnn_bfs_action_last.pt"
...
def protocol_audit(args, out_dir: Path):
    issues=[]
    if not str(SCRIPT_PATH).replace('\\','/').endswith('train_1_0_0_cnn_bfs_action_extdata_v3.py'):
        issues.append({"scope":"script_name","status":"FAIL","message":"script must use extdata_v3 name"})
...
def _distribution_candidate_info(records, args):
    info = []
    skipped = []
    for idx, rec in enumerate(records):
        try:
            dist = vc.bfs_distances(rec["maze"], tuple(rec["goal"]))
            path = vc.shortest_path(rec["maze"], tuple(rec["start"]), tuple(rec["goal"]), dist)
            if not path:
                skipped.append({"sample_id": rec.get("sample_id"), "row_index": rec.get("row_index"), "reason": "unsolvable"})
                continue
            bfs_len = len(path) - 1
            diff = _difficulty_from_bfs_len(args, bfs_len)
            rec = dict(rec)
            rec["recomputed_bfs_len"] = bfs_len
            rec["recomputed_difficulty"] = diff
            info.append(rec)
        except Exception as e:
            skipped.append({"sample_id": rec.get("sample_id"), "row_index": rec.get("row_index"), "reason": str(e)})
    return info, skipped
...
def _select_train_records_by_distribution(records, args):
    policy = str(getattr(args, "train_distribution_policy", "source_order"))
    max_n = getattr(args, "max_train_mazes", None)
    if max_n is None:
        max_n = getattr(args, "max_external_samples", None)
    max_n = int(max_n) if max_n is not None else len(records)
    rng = __import__("random").Random(int(getattr(args, "train_distribution_seed", getattr(args, "seed", 42))))
    candidates, skipped = _distribution_candidate_info(records, args)
...
def _print_default_train_contract(args, dist_report):
    ...
    print(f"max_external_samples={getattr(args, 'max_external_samples', None)}")
    print(f"max_train_mazes={getattr(args, 'max_train_mazes', None)}")
...
def _print_v1_run_summary(args, dist_report, evaluation_summary, plan_path):
    print("[RUN SUMMARY]")
    ...
    print("no_fallback_corridor=true")
    print("[V1 METRICS]")
    print(f"rollout_success_rate={evaluation_summary.get('rollout_success_rate')}")
    print(f"shortest_path_success_rate={evaluation_summary.get('shortest_path_success_rate')}")
    print(f"avg_bfs_gap={evaluation_summary.get('avg_bfs_gap')}")
    print("[POST CHECK]")
...
def _write_distribution_ablation_summary_v1(output_root: Path) -> None:
    """Write cumulative v1 distribution ablation summary by scanning sibling run dirs."""
    rows = []
    for run_dir in sorted(Path(output_root).iterdir()) if Path(output_root).exists() else []:
        ...
        meta = _read_json_if_exists(run_dir / "evaluation_summary.json")
        ...
        row = {
            ...
            "action_accuracy_hard_label": meta.get("action_accuracy_hard_label") if isinstance(meta, dict) else None,
            "action_accuracy_best_action_set": meta.get("action_accuracy_best_action_set") if isinstance(meta, dict) else None,
            "rollout_success_rate": meta.get("rollout_success_rate") if isinstance(meta, dict) else None,
            "shortest_path_success_rate": meta.get("shortest_path_success_rate") if isinstance(meta, dict) else None,
            "avg_bfs_gap": meta.get("avg_bfs_gap") if isinstance(meta, dict) else None,
            "sample_check_output": None,
            "handdraw_check_output": None,
            "sample_check_success": None,
            "handdraw_check_success": None,
            "handdraw_selected_count": None,
            "gif_report_path": None,
        }
        if isinstance(plan, dict):
            ...
            for key, success_key in (("sample_check_output", "sample_check_success"), ("handdraw_check_output", "handdraw_check_success")):
                p = row.get(key)
                summ = _read_json_if_exists(Path(p) / "test_rollout_summary.json") if p else None
                if isinstance(summ, dict):
                    row[success_key] = summ.get("rollout_success_rate")
                    if key == "handdraw_check_output":
                        row["handdraw_selected_count"] = summ.get("num_tested_records")
                        gif_path = Path(p) / "test_gif_examples_report.json"
                        row["gif_report_path"] = str(gif_path) if gif_path.exists() else None
        rows.append(row)
    payload = {
        "summary_type": "v1_distribution_ablation_cumulative",
        "diagnostic_only": True,
        "runs": rows,
        "note": "Generated by scanning output dirs. Absence of a run means it has not completed or lacks train_distribution_report.json.",
    }
    ext.save_json(Path(output_root) / "distribution_ablation_summary_v1.json", payload)
    md = ["# V1 Distribution Ablation Summary", "", "Diagnostic only. Success values are not final model conclusions.", ""]
    md.append("| run_name | policy | selected_counts | rollout_success | shortest_path_success | avg_bfs_gap | sample_check | handdraw_check |")
    md.append("|---|---|---:|---:|---:|---:|---:|---:|")
    for r in rows:
        md.append(f"| {r.get('run_name')} | {r.get('train_distribution_policy')} | {r.get('selected_counts_by_difficulty')} | {r.get('rollout_success_rate')} | {r.get('shortest_path_success_rate')} | {r.get('avg_bfs_gap')} | {r.get('sample_check_success')} | {r.get('handdraw_check_success')} |")
    (Path(output_root) / "distribution_ablation_summary_v1.md").write_text("\n".join(md), encoding="utf-8")
...
def _run_default_post_train_evaluations(args, train_out_dir: Path, hist: Dict[str, Any], device) -> None:
    ...
    if not getattr(args, "auto_final_evaluation", True):
        return
    ckpt = hist.get("checkpoint_best") or str(train_out_dir / "1_0_0_extdata_v3_cnn_bfs_action_best.pt")
    ...
    if getattr(args, "sample_check_dataset_path", None):
        ...
        t.run_name = getattr(args, "sample_check_run_name", "default_sample_check_extdata_v3")
    ...
    if getattr(args, "handdraw_dataset_path", None) and Path(args.handdraw_dataset_path).exists():
        ...
        t.split = getattr(args, "handdraw_split", "all"); t.sample_policy = "sequential"; t.difficulty_filter = "all"; t.bucket_filter = "all"
        t.max_test_samples = int(getattr(args, "handdraw_check_max_test_samples", 10000))
        t.run_name = getattr(args, "handdraw_check_run_name", "default_handdraw_check_extdata_v3")
...
def main():
    ...
    train_records, train_distribution_report = _select_train_records_by_distribution(converted, args)
    ...
    if not train_records:
        ext.save_json(out_dir/"blocking_report.json", {"status":"BLOCKED", "found":"train distribution selected zero records", "affected_goal":"v1 distribution ablation training", "risk_if_continue":"would train on empty data", "needs_upper_confirmation":"change train_distribution_policy or max_train_mazes", "suggested_next_agent":"decision / code"})
        return
    samples, maze_records, label_report = external_records_to_bfs_samples(train_records,args)
    label_report["train_distribution_policy"] = train_distribution_report.get("train_distribution_policy")
    label_report["input_external_mazes_after_distribution_selection"] = len(train_records)
    ...
    if args.dry_run_dataset_only:
        ...
        return
    train_samples,val_samples,test_samples=split_samples_by_maze(train_records,samples,args.split_manifest_path)
    ...
    rollouts, rollout_summary=rollout_external(model,train_records,args,device)
    evaluation_summary={
    ...
    write_baseline_comparison(out_dir)
    ext.save_json(out_dir/"run_metadata.json", {"version":VERSION,"script":str(SCRIPT_PATH),"model_summary":model.summary(),"started_at":datetime.now(timezone.utc).isoformat(),"external_dataset_path":args.external_dataset_path,"output_dir":str(out_dir),"n_train_samples":len(train_samples),"n_val_samples":len(val_samples),"n_test_samples":len(test_samples),"train_distribution_policy":train_distribution_report.get("train_distribution_policy"),"train_distribution_report_path":str(out_dir/"train_distribution_report.json"),"default_auto_final_evaluation":bool(getattr(args,"auto_final_evaluation",True))})
    _run_default_post_train_evaluations(args, out_dir, hist, device)
    _write_distribution_ablation_summary_v1(V1_ROOT / "outputs" / "1.0.0_data_swap_3_0_4")
    _print_v1_run_summary(args, train_distribution_report, evaluation_summary, out_dir/"default_post_train_evaluation_plan.json")
