...
VERSION = "v2.0.7debug_rm_context_probe_fixed"
SIZE = 8
...
def generate_pair_candidates_for_maze(families: List[Family], args: argparse.Namespace) -> Dict[str, List[PreferencePair]]:
    ...
    for fam in families:
        ...
        add_candidate_pair(cands, R, V, "outcome", "family_mandatory", args)
        add_candidate_pair(cands, R, L, "outcome", "family_mandatory", args)
        # wall: timeout with wall is worse than no-wall timeouts.
        add_candidate_pair(cands, E, W, "wall_presence", "family_mandatory", args)
        add_candidate_pair(cands, V, W, "wall_presence", "family_mandatory", args)
        add_candidate_pair(cands, L, W, "wall_presence", "family_mandatory", args)
        ...
        add_candidate_pair(cands, R, V, "revisit_loop", "family_mandatory", args)
        ...
        add_candidate_pair(cands, V, L, "revisit_loop", "family_mandatory", args)
...
def print_expected_gradient_pressure(args: argparse.Namespace) -> None:
    rows = [
        ("outcome", 4, 0.70),
        ("wall_presence", 3, 0.36),
        ("wall_exp", 3, 0.18),
        ("revisit_loop", 4, 0.18),
        ("path", 2, 0.05),
        ("cpl_recover_wall", 1, 0.36),
        ("cpl_explore_wall", 1, 0.30),
        ("cpl_recover_loop", 1, 0.20),
        ("cpl_visit_toward", 1, 0.14),
    ]
    pressures = [(name, n, m, n*m) for name, n, m in rows]
    total = sum(p for *_rest, p in pressures) or 1.0
    print("\n[Expected Gradient Pressure]")
    print(f"{'bucket':22s} {'n_per_family':>12s} {'typical_margin':>15s} {'pressure':>10s} {'pct':>8s}")
    for name, n, m, p in pressures:
        print(f"{name:22s} {n:12d} {m:15.3f} {p:10.3f} {100*p/total:7.2f}%")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output-dir", default="./v2.0.7debug")
...
def main() -> None:
    ...
    if args.mode == "all":
        print("[INFO] v2.0.7debug is RM-only. QCNN training is disabled. Interpreting --mode all as --mode all-rm.")
        args.mode = "all-rm"
    set_seed(args.seed)
    ...
    print("RM-only debug: trajectory families, bucket margins, contextual probes, greedy RM rollouts. QCNN training is disabled.")
