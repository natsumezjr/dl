...
class TrainConfig:
    ...
    # v2.1.3: BFS demonstration replay + n-step demo targets
    demo_ratio: float = 0.25
    demo_paths_per_episode: int = 1
    demo_recent_capacity: int = 20_000

    # v2.1.3: n-step targets are applied to demo_buffer transitions.
    # Online rollout transitions remain ordinary 1-step DQN transitions.
    n_step: int = 3
    ...
    output_dir: str = "./v2.1.3"
    output_name: str = "v2.1.3_n_step_demo_replay"
...
def optimize_model(
    ...
    n_steps = torch.tensor(batch.n_steps, dtype=torch.float32, device=device).unsqueeze(1)
    ...
    with torch.no_grad():
        ...
        discount = torch.pow(torch.tensor(cfg.gamma, dtype=torch.float32, device=device), n_steps)
        target_q_values = rewards + discount * next_q_values * (1.0 - dones)
...
def build_bfs_demo_transitions(
    ...
    """
    v2.1.3 BFS demonstration replay with n-step demo targets.

    Important boundary:
      - This is still v2.1.x demonstration replay.
      - Demo samples still go through demo_buffer.
      - The online rollout path is NOT separately converted to n-step here.
      - Only BFS demo transitions are converted into n-step returns.

    For a demo path s_i -> s_{i+1} -> ... -> goal, each transition stores:
      action = first BFS action at s_i
      reward = r_i + gamma*r_{i+1} + ... + gamma^{k-1}*r_{i+k-1}
      next_state = s_{i+k}
      n_steps = k

    optimize_model then uses:
      target = reward + gamma^n_steps * Q_target(next_state, argmax Q_policy(next_state))
    unless done=True.
    """
    path = bfs_shortest_path(grid, start, goal)
    ...
    n = max(1, int(cfg.n_step))

    for i in range(len(path) - 1):
        pos = path[i]
        first_next = path[i + 1]
        action = action_between_positions(pos, first_next)

        # Accumulate up to n rewards along the BFS expert path.
        reward_sum = 0.0
        actual_n = 0
        done = False
        final_pos = first_next

        max_j = min(i + n, len(path) - 1)
        for j in range(i, max_j):
            nxt = path[j + 1]
            step_done = nxt == goal
            step_reward = cfg.step_penalty + (cfg.goal_reward if step_done else 0.0)
            reward_sum += (cfg.gamma ** actual_n) * step_reward
            actual_n += 1
            final_pos = nxt
            if step_done:
                done = True
        ...
        next_state = state_from_grid_pos_goal(grid, final_pos, goal)

        old_dist = bfs_distance(grid, pos, goal)
        new_dist = bfs_distance(grid, first_next, goal)
        progress = (
        ...
        transitions.append(
            make_transition(
                ...
                maze_type=maze_type + "_demo_nstep",
...
def train_one_experiment(
    ...
    print(f"n_step={cfg.n_step} (applied to demo_buffer transitions only)")
...
def make_save_name(args) -> str:
    output_name = getattr(args, "output_name", "") or "v2.1.0_bfs_demo_replay"
    return f"{output_name}.pt"


def build_cfg_from_args(args) -> TrainConfig:
    output_name = args.output_name or "v2.1.3_n_step_demo_replay"
    cfg = TrainConfig(
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./v2.1.3")
    parser.add_argument("--output-name", type=str, default="v2.1.3_n_step_demo_replay")
    ...
    parser.add_argument("--n-step", type=int, default=3, help="n-step return length for BFS demo_buffer transitions only")
