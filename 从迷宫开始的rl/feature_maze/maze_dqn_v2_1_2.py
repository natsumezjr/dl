...
class TrainConfig:
    ...
    # v2.1.2: expert warmup phase before the original easy/medium/hard stages.
    # Warmup uses mixed random-maze difficulties: easy 0.4, medium 0.4, hard 0.2.
    demo_warmup_episodes: int = 1000
    demo_warmup_epsilon: float = 0.05
    ...
    output_dir: str = "./v2.1.2"
    output_name: str = "v2.1.2_1_expert_warmup_demo_only"
...
def warmup_difficulty_by_episode() -> str:
    """v2.1.2 expert warmup difficulty mix: easy 0.4, medium 0.4, hard 0.2."""
    return weighted_choice({"easy": 0.40, "medium": 0.40, "hard": 0.20})


def stage_difficulty_by_episode(ep: int) -> str:
    """Original fixed three-stage curriculum after warmup: 1-1000 easy, 1001-2000 medium, 2001-3000 hard."""
    if ep <= 1000:
        return "easy"
    if ep <= 2000:
        return "medium"
...
def sample_train_maze(cfg: TrainConfig, ep: int) -> Tuple[np.ndarray, Tuple[int, int], Tuple[int, int], str, str]:
    if cfg.maze_schedule == "stage":
        if ep <= cfg.demo_warmup_episodes:
            difficulty = warmup_difficulty_by_episode()
            return generate_random_maze(difficulty, fixed_sg_prob=0.0)

        main_ep = ep - cfg.demo_warmup_episodes
        difficulty = stage_difficulty_by_episode(main_ep)
        return generate_random_maze(difficulty, fixed_sg_prob=0.0)
...
def epsilon_for_training_phase(ep: int, cfg: TrainConfig) -> float:
    """
    v2.1.2 keeps the original epsilon schedule for the original
    easy/medium/hard stages, but uses low exploration during expert warmup.
    """
    if cfg.maze_schedule == "stage" and ep <= cfg.demo_warmup_episodes:
        return cfg.demo_warmup_epsilon

    if cfg.maze_schedule == "stage":
        return epsilon_by_episode(ep - cfg.demo_warmup_episodes, cfg)

    return epsilon_by_episode(ep, cfg)
...
def train_one_experiment(
    ...
    print("Warmup is DEMO-ONLY: no online action choice, no random rollout transitions.")
    print(f"expert_warmup_episodes={cfg.demo_warmup_episodes}")
    print("warmup_difficulty_mix=easy:0.40, medium:0.40, hard:0.20")
    ...
    for ep in range(1, cfg.episodes + 1):
        ...
        warmup_demo_only = (cfg.maze_schedule == "stage" and ep <= cfg.demo_warmup_episodes)
        epsilon = epsilon_for_training_phase(ep, cfg)

        if warmup_demo_only:
            # v2.1.2(1): expert warmup is demo-only.
            # During the first 1000 episodes, do not let the online policy choose
            # actions and do not insert random/wall-hit rollout transitions.
            # The replay buffer receives only BFS demonstration transitions, then
            # the model is optimized from that expert-heavy distribution.
            updates_this_episode = cfg.max_steps
            ...
            demo_reward = sum(float(tr.reward) for tr in demo_transitions)
            demo_len = max(1, len(demo_transitions))

            episode_rewards.append(demo_reward)
            success_flags.append(1 if demo_transitions else 0)
            wall_hit_rates.append(0.0)
            invalid_action_rates.append(0.0)
            repeat_counts.append(0)
            progress_rates.append(1.0 if demo_transitions else 0.0)
            path_ratios.append(1.0 if demo_transitions else np.nan)

            global_step += demo_len
        ...
        if ep % 100 == 0:
            ...
            print(
                ...
                f"phase={'demo_only' if (cfg.maze_schedule == 'stage' and ep <= cfg.demo_warmup_episodes) else 'online':9s} "
                f"eps={epsilon:.3f} "
...
def make_save_name(args) -> str:
    output_name = getattr(args, "output_name", "") or "v2.1.2_1_expert_warmup_demo_only"
    return f"{output_name}.pt"


def build_cfg_from_args(args) -> TrainConfig:
    output_name = args.output_name or "v2.1.2_1_expert_warmup_demo_only"
    cfg = TrainConfig(
        ...
        demo_warmup_episodes=args.demo_warmup_episodes,
        demo_warmup_epsilon=args.demo_warmup_epsilon,
    )
    apply_reward_preset(cfg)
    return cfg
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./v2.1.2")
    parser.add_argument("--output-name", type=str, default="v2.1.2_1_expert_warmup_demo_only")
    parser.add_argument("--demo-warmup-episodes", type=int, default=1000)
    parser.add_argument("--demo-warmup-epsilon", type=float, default=0.05)
